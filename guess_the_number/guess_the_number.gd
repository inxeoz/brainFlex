...
func next_lvl_values():
	...
	GtnGlobal.box_values[3] = values[1] # not 0 index because second time the 0th index value
...
func _on_button_reset_pressed():
	...
	next_lvl_values()
	start_timer(Global.timer_time)
	
	GtnGlobal.hide_num = GtnGlobal.possi[randi_range(0, 3)]
	print("hide numnis ", GtnGlobal.hide_num)
	for i in range(0, GtnGlobal.color_nodes.size()):
		if i != GtnGlobal.hide_num:
...
func _on_keyboard_value_entered():
	...
	if GtnGlobal.box_values[GtnGlobal.hide_num] == str(GtnGlobal.value):
		$Timer.stop()
		print("you got it right ", GtnGlobal.box_values[GtnGlobal.hide_num])
		print(" time ", time)
		...
		print("you got it wrong ", GtnGlobal.box_values[GtnGlobal.hide_num])
